...
export function ShareModal({ summary, onClose }: { summary: ProfileSummary; onClose: () => void }) {
  ...
  const slug = summary.customUrl || summary.handle;
  const url = `https://harbor.site/u/${slug}`;
  const cardUrl = `${url}/card.png`;
  const text = `${summary.alias} on Harbor`;
  const socials = socialUrls(url, text);
  const embeds = embedFields(url, cardUrl, summary.alias);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-edge-soft px-5 py-4">
          <h2 className="font-display text-[19px] text-ink">Share profile</h2>
          <button
            onClick={onClose}
            aria-label="Close"
        ...
        <div className="flex flex-col gap-4 px-5 pb-5 pt-4">
          {tab === "link" ? (
            <>
              <CopyRow label="Profile link" value={url} primary />
              <div className="grid grid-cols-3 gap-2">
                ...
                <ShareButton label="Email" onClick={() => openUrl(socials.email)}>
                  <Mail size={17} />
                </ShareButton>
                <ShareButton
                  label={discordCopied ? "Copied for Discord" : "Copy for Discord"}
